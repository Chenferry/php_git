...
//读取值原始串
static int iniGetValue(const char *section, const char *key, unsigned char *value, int maxlen, const char *defvalue)
{
	int type;
	char content[SIZE_LINE];
	char *rem1, *rem2, *nextline;
...
//获取字符串，不带引号
int iniGetString(const char *section, const char *key, unsigned char *value, int maxlen, const char *defvalue)
{
	int ret;
	int len;
    ...
    else if (value[len - 1] == '\'' || value[len - 1] == '\"')
    {
		memmove(value, value, len - 1);
    }
	return ret;
}
...
//获取整数值
unsigned int iniGetInt(const char *section, const char *key, int defvalue)
{
	unsigned char valstr[64];
...
int iniSetString(const char *section, const char *key, const unsigned char *value)
{
	FILE *file;
...
int iniSetInt(const char *section, const char *key, unsigned int value, int base)
{
	unsigned char valstr[64];
