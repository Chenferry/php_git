...
include_once(__DIR__ . '/mzPush/autoload.php');
...
class MEIZU
{
    ...
    {
        $mzPush = new MzPush($aksk['AppID'], $aksk['AppSecret']);
        $data = json_encode(array("a" => "aaa", "b" => "bbb"));
        //通知栏消息对象
        $varnishedMessage = new VarnishedMessage();
        $varnishedMessage->setTitle($title)->setContent($data)->setClickType(2)->setUrl('http://www.baidu.com/')->setNoticeExpandType(1)->setNoticeExpandContent('扩展内容')->setOffLine(1);
        var_dump($mzPush->varnishedPush(array($token), $varnishedMessage));
    }
}
